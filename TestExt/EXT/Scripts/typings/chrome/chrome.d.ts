...
declare module chrome.identity {
	...
	var getProfileUserInfo: (callback: (userInfo: { email: string; id: string }) => void) => void;
	var removeCachedAuthToken: (options: any, callback: () => void) => void;
	...
	* When the provider redirects to a URL matching the pattern https://<app-id>.chromiumapp.org/*, the window will close, and the final redirect URL will be passed to the callback function. For a good user experience it is important interactive auth flows are initiated by UI in your app explaining what the authorization is for. Failing to do this will cause your users to get authorization requests with no context. In particular, do not launch an interactive auth flow when your app is first launched.
	* uses:{object} details - string url The URL that initiates the auth flow. boolean	(optional) interactive Whether to launch auth flow in interactive mode.
	* Since some auth flows may immediately redirect to a result URL, launchWebAuthFlow hides its web view until the first navigation either redirects to the final URL, or finishes loading a page meant to be displayed.If the interactive flag is true, the window will be displayed when a page load completes. If the flag is false or omitted, launchWebAuthFlow will return with an error if the initial navigation does not complete the flow.
	*/
	var launchWebAuthFlow: (details:WebAuthFlowDetails , callback: (redirectUrl: string) => void) => void;

	/**
	* Callback_url will be appended to the redirect url, for instance a callback
	* url of "provider_cb" will return a url of: 
	* "https://<app-id>.chromiumapp.org/provider_cb"
	*/
	var getRedirectURL: (callback_url?: string) => string;
}
